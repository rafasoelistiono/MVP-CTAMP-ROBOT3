...
def _arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run CTAMP v3 confirmed cube stacking."
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Run v3 TMM search/confirmation without MuJoCo execution",
    )
...
def main() -> int:
    ...
    output = (
        args.output
        or args.log_dir / f"stacking_wall_world_v3_{time.strftime('%Y%m%d_%H%M%S')}"
    )
